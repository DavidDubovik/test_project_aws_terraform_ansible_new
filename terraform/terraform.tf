...
resource "aws_instance" "my_project" {                 # тут ми створює віртуальну машину (EC2-інстанс) у AWS. Та вказуємо інікальне имя ресурса  
  ...
  instance_type   = "t3.micro"                         # Визначає тип EC2-інстансу, тобто апаратні характеристики (процесор, пам’ять, мережа тощо). У коді: "t3.micro" — це один із найменших і найдешевших типів інстансів (1 vCPU, 1 ГБ оперативної пам’яті), який підходить для легких задач, тестування чи розробки.
  ...
  provisioner "local-exec" {                       # це механізм у Terraform, який дозволяє виконувати додаткові дії після створення, local-exec: Виконує команду локально на машині, де запущений Terraform.
    command = "echo ${self.public_ip} > terraform.tfstate" # тут команда яку треба виконати після створення ресурса, она зберігає ip ресурса в файл hosts.txt
