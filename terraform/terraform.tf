...
resource "aws_instance" "my_project" {                 # тут ми створює віртуальну машину (EC2-інстанс) у AWS. Та вказуємо інікальне имя ресурса  
  ami             = var.ami_id != "" ? var.ami_id : "ami-07e075f00c26b085a"         # тут ми вказуємо шаблони операціїних систем та програмного забеспечення
  instance_type   = var.instance_type != "" ? var.instance_type : "t3.micro"                        # Визначає тип EC2-інстансу, тобто апаратні характеристики (процесор, пам’ять, мережа тощо). У коді: "t3.micro" — це один із найменших і найдешевших типів інстансів (1 vCPU, 1 ГБ оперативної пам’яті), який підходить для легких задач, тестування чи розробки.
